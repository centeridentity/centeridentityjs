...
export default class CenterIdentity {
  constructor(kwargs) {
    ...
    this.friends_list_wif =
      "Kx5or1SpDjQRy2gFwUpkGtxVZaM9tASYpozkb33TErm2PDBx38nJ";
    this.origin =
  ...
  async createRelationshipTransaction(me, user, group, extra_data) {
    ...
    var encryptedRelationship = await this.encrypt(
      me.wif,
      JSON.stringify(join.relationship)
    );
    var meObject = this.toObject(me);
    var requested_rid = null;
    var requester_rid = null;
    if (group) {
      requested_rid = this.generate_rid(group, user);
      requester_rid = this.generate_rid(group, me);
    }
  ...
  async set(user, latitude, longitude) {
    return new Promise(
      ...
      .then(
        function (encryptedSeed) {
          return new Promise(
            async function (resolve, reject) {
              if (this.selfGenerateTransaction) {
                var centerIdentityUser = {
                  public_key:
                    "02a9aed3a4d69013246d24e25ded69855fbd590cb75b4a90fbfdc337111681feba",
                ...
                var join = await this.createRelationship(
                  public_user,
                  centerIdentityUser
                );
  ...
  async setFromNew(username, latitude, longitude) {
    ...
    return this.createUser(username)
      .then(
        function (user) {
          this.user = user;
          return this.set(this.user, this.latitude, this.longitude);
        }.bind(this)
      )
      .then(
        function () {
  ...
  async setV2(user, entropy) {
    return new Promise(
      function (resolve, reject) {
        this.user = user;
        this.entropy = entropy;
        return resolve(entropy);
      }.bind(this)
    )
      .then(
        function (entropy) {
          return this.showPositionV2(entropy);
        }.bind(this)
      )
      .then(
        function (entropy) {
  ...
  async setFromNewV2(username, entropy) {
    this.username = username;
    this.entropy = entropy;
    return this.createUser(username)
      .then(
        function (user) {
          this.user = user;
          return this.setV2(this.user, this.entropy);
        }.bind(this)
      )
      .then(
        function () {
  ...
  async get(username, latitude, longitude) {
    return new Promise(
      ...
      .then(
        function (entropy) {
          return this.showPosition(entropy);
        }.bind(this)
      )
      .then(
        function () {
  ...
  async showPositionV2(entropy) {
    return new Promise(
      function (resolve, reject) {
        this.header = entropy + this.api_key;
        return resolve();
      }.bind(this)
    );
  }
    ...
    return new Promise(
      function (resolve, reject) {
        for (var i = 0; i === i; i++) {
          ...
          if (
            parseInt(this.header, 16) < this.strength &&
            !this.symmetric_key
          ) {
            this.header = forge.sha256
              .create()
  ...
  encrypt(keyStr, message) {
    var key = forge.pkcs5.pbkdf2(
      forge.sha256.create().update(keyStr).digest().toHex(),
      "salt",
      400,
      32
    );
      ...
      function (resolve, reject) {
        fetch(this.url_prefix + "/digup", {
          ...
          .then(async (result) => {
            ...
            let decryptedData = this.decrypt(
              this.symmetric_key,
              data.relationship
            );
  ...
  createUser(username) {
    return new Promise(
      async function (resolve, reject) {
        ...
        return resolve({
          username_signature: await this.generate_username_signature(
            key,
            username
          ),
          username: username,
  ...
  signIn(session_id, user, signin_url) {
    return this.signSession(session_id, user).then(
      function (signature) {
        return new Promise(
          async function (resolve, reject) {
  ...
  signInWithLocation(
    ...
    return fetch(
      ...
      .then(
        async function (user) {
          public_user = await this.reviveUser(user.wif, public_username);
  ...
  get_dh_keys(me, them) {
    var raw_dh_private_key = forge.sha256
      .create()
      .update(me.wif + them.username_signature)
      .digest()
      .toHex();
    var raw_dh_public_key = X25519.getPublic(
      this.hexToByteArray(raw_dh_private_key)
    );
    return {
  ...
  getSharedSecret(me, them, their_txn) {
    ...
    var privk = new Uint8Array(
      dh_keys.dh_private_key.match(/[\da-f]{2}/gi).map(function (h) {
        return parseInt(h, 16);
      })
    );
    var pubk = new Uint8Array(
      their_txn.dh_public_key.match(/[\da-f]{2}/gi).map(function (h) {
        return parseInt(h, 16);
      })
    );
  ...
  async authenticate(challenge_url = "") {
    ...
    const result = await fetch(challenge_url || "/challenge", {
      method: "POST",
      body: JSON.stringify({
        api_key: this.api_key,
  ...
  generate_rid(user1, user2, extra_data = "") {
    var username_signatures = [
      user1.username_signature,
      user2.username_signature,
    ].sort(function (a, b) {
      return a.toLowerCase().localeCompare(b.toLowerCase());
    });
    return forge.sha256
      .create()
      .update(username_signatures[0] + username_signatures[1] + extra_data)
      .digest()
      .toHex();
  }
  ...
  messageFromEncryptedTransaction(me, them, their_txn, txn) {
    ...
    var relationship = JSON.parse(
      this.decrypt(shared_secret, copy_txn.relationship)
    );
    return relationship;
  }
  ...
  async generatePrivateMessageTransaction(
    me,
    them,
    their_txn,
    message,
  ...
  async getTransactionsByRequestedRid(rid) {
    return fetch(
      this.url_prefix + "/get-transaction-by-rid?requested_rid=" + rid
    );
  }

  async getTransactionsByRequesterRid(rid) {
    return fetch(
      this.url_prefix + "/get-transaction-by-rid?requester_rid=" + rid
    );
  }
  ...
  async getDataByCollection(me, collection, friendList) {
    ...
    var collection = await Promise.all(
      txns.map(async (txn) => {
        var copy_txn = JSON.parse(JSON.stringify(txn));
        var relationship = JSON.parse(
          this.decrypt(me.wif, copy_txn.relationship)
        );
        return relationship;
      })
    );
  ...
  async getFriendsLists(me, listName) {
    ...
    return this.getTransactionsByRid(rid).then(async (txns) => {
      return txns;
    });
  }
  ...
  async getFriendsList(me, listName) {
    ...
    return this.getTransactionsByRid(rid).then(async (txns) => {
      if (txns.length === 0) {
        var friendsList = await this.reviveUser(
          this.friends_list_wif,
          "default"
        );
        return this.createRelationshipTransaction(
          me,
          friendsList,
          null,
          me.wif + ":all_friends_lists"
        );
      }
      for (var i = 0; i < txns.length; i++) {
      ...
      .then(async (rels) => {
        ...
        for (var i = 0; i < txn.length; i++) {
          if (txn[i].dh_public_key === "") {
            ...
            var theirIdentity = this.theirIdentityFromEncryptedTransaction(
              me,
              myRel
            );
            var message = this.messageFromEncryptedTransaction(
              me,
              theirIdentity,
              theirRel,
              messageTxn
  ...
  async sendPrivateMessage(me, them, collection, message) {
    ...
    return this.getRelationshipTransactions(me, them)
      .then(async (rels) => {
        ...
        return await fetch(
          this.url_prefix + "/transaction?username_signature=fu&" + this.origin,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  async issueCredential(me, them, credential, credentialMessage) {
    ...
    return {
      transaction: await this.sendPrivateMessage(
        me,
        them,
        collection,
        relationshipMessage
  ...
  async getCredentialsIssued(me, them, credential) {
    return this.getPrivateMessages(me, them, "credential_issues", credential);
  }

  async getCredentialsRequested(me, them, credential) {
    return this.getPrivateMessages(me, them, "credential_requests", credential);
  }

  async requestCredentialFromIssuer(me, issuer, credential) {
    // me = subject
  ...
  async requestCredentialThroughSubject(me, issuer, subject, credential) {
    // me = verifier
    var collection = "credential_requests";
    Object.assign(credential, {
      issuer: this.toObject(issuer),
  ...
  async forwardIssuedCredential(me, issuer, verifier, credential) {
    // me = subject, them = verifier
    var signedCredential = await this.sign(JSON.stringify(credential), me);
    var message = JSON.stringify(
      Object.assign(credential, {
        subject_signature: signedCredential,
  ...
  async forwardRequestedCredential(me, issuer, verifier, credential) {
    // me = subject, them = issuer
    var signedCredential = await this.sign(JSON.stringify(credential), me);
    var message = JSON.stringify(
      Object.assign(credential, {
        subject_signature: signedCredential,
  ...
  async getIdentityLink(identity) {
    return fetch(
        ...
        encodeURIComponent(identity.username_signature),
      {
        ...
        body: JSON.stringify({
          file: btoa(JSON.stringify(this.toObject(identity))),
        }),
      }
    ).then(async (res) => {
  ...
  async getCredentialLink(credential) {
    return fetch(
      this.url_prefix +
        "/sia-upload?filename=" +
        encodeURIComponent(credential.identity.username_signature),
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async connectIdentities(me, them, friendList, collection, extra_data) {
    if (!friendList) {
      friendList = await this.reviveUser(
        this.friends_list_wif,
        collection || "default"
      );
    }
    var myRel = await this.createRelationshipTransaction(
      me,
      them,
      friendList,
      extra_data
  ...
  async importConnectionFromSkylink(
    me,
    skylink,
    collection,
    friendList,
    ...
    var group = await this.reviveUser(
      this.friends_list_wif,
      collection || "default"
    );
    if (friendList) {
    ...
    return fetch("https://siasky.net/" + skylink).then(async (res) => {
